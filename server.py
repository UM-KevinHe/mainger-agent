...
Endpoints
---------
  GET  /api/config                          # vendor list + current defaults
  POST /api/sessions/init     (SSE)         # upload initial data + first message
  POST /api/sessions/{id}/chat (SSE)        # follow-up message (with optional uploads)
  GET  /api/sessions/{id}                   # retrieve current state
...
#   - models:    suggested model names (UI presents as a dropdown; user can type a custom one too)
#   - base_url:  default base URL the server should use if user doesn't override
#   - needs_url: whether the user must supply a base_url (true for 'custom' only)
#   - kind:      "closed" or "open" (informational, controls grouping in UI)
VENDOR_CATALOG: dict[str, dict[str, Any]] = {
    ...
    "together": {
        "label": "Together AI (open-source hosted)",
        "kind": "open",
        "models": [
    ...
    "fireworks": {
        "label": "Fireworks AI (open-source hosted)",
        "kind": "open",
        "models": [
    ...
    "huggingface": {
        "label": "HuggingFace Inference Endpoints",
        "kind": "open",
        "models": [
    ...
    "custom": {
        "label": "Custom (your own OpenAI-compatible endpoint)",
        "kind": "open",
...
def get_config() -> dict:
    cfg = _read_config()
    # Trim catalog payload for the UI
    catalog = {
...
def _run_turn_generator(state: SessionState, user_msg: str, initial: bool):
    def gen():
        ...
        # Pass api_key + base_url into the agent. We monkey-patch the cfg
        # locally so run_agent_streaming sees the override; the canonical cfg
        # in state.cfg is unaltered.
        cfg = dict(state.cfg)

        try:
            # The agent already accepts api_key; we extend its kwargs by
            # injecting base_url into cfg so make_client can pick it up.
            # See make_client's resolution rules.
            iterator = _run_with_base_url(
...
def _run_with_base_url(session, user_msg, cfg, *, api_key, base_url, prior_messages):
    """Thin wrapper that delegates to run_agent_streaming, passing base_url
    via cfg so it reaches make_client."""
    # Inject base_url so the agent's call to make_client can use it.
    # run_agent_streaming reads cfg['vendor'], cfg['model'], etc. We add
    # cfg['base_url'] which make_client will pick up via kwarg.
    # The agent itself doesn't read base_url; we patch make_client at the
    # call site instead by importing and calling directly.
    return run_agent_streaming(session, user_msg, cfg,
                               api_key=api_key, prior_messages=prior_messages,
                               base_url=base_url)
