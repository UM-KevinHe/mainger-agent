...
You probably do not need to read every section. Pick the path below that
...
flowchart TD
    ...
    S1 --> S2[Section 2 — Install]
    S2 --> S3a[Section 3 — Get one API key]
    S3a --> S4[Section 4 — Configure]
    S4 --> S5[Section 5 OR 6 — Run]

    DeployPath[<b>Path B: Deploy</b><br/>~30 min]
    DeployPath --> SAll[Sections 1–4 — same as Path A]
    SAll --> S6[Section 6 — Web UI in detail]
    S6 --> S7[Section 7 — Output files]
    S7 --> S10[Section 10 — Privacy and security]
    S10 --> S9[Section 9 — Troubleshooting,<br/>as needed]

    ContribPath[<b>Path C: Extend</b><br/>~1 hour]
    ContribPath --> SAllC[Sections 1–4 — same as Path A]
    SAllC --> SAppendix[Appendix — extension points]
    SAppendix --> S8[Section 8 — Choosing a model<br/>for development]
    S8 --> S9c[Section 9 — Troubleshooting]

    style TryPath fill:#dbeafe,stroke:#2563eb,color:#000
    style DeployPath fill:#dcfce7,stroke:#16a34a,color:#000
    style ContribPath fill:#fef3c7,stroke:#ca8a04,color:#000
```

**Path A: Quickstart** (Sections 1–4, then 5 or 6, ~10 minutes). Use this
if you want to run an analysis on your own data and see what the tool
produces. Skip the deep installation, deployment, and extension sections.

**Path B: Deploy and share** (Sections 1–4, 6, 7, 10, plus 9 as needed,
~30 minutes). Use this if you want to set up the web UI for collaborators
to use, and need to think carefully about what data flows where.

**Path C: Extend the tool** (Sections 1–4, then the Appendix, ~1 hour).
Use this if you want to modify the agent's behavior, add a new tool, or
adapt the system prompt to a different domain.

If you just want to know whether the tool will work for your problem,
read **Section 11** ("What the agent does, step by step") first; it is a
...
Designed for **single-user local use**. The server stores session state
in memory and accepts API keys via the form. Do not expose this server
beyond `127.0.0.1` without changing the auth model.
